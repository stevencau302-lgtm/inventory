  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold" style={{ color: 'var(--color-text)' }}>Kategori</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>{categories.length} kategori tersedia</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {categories.map(cat => {
          ...
          return (
            <div key={cat.id} className="glass-card p-6 group">
              ...
              <h3 className="font-semibold mt-4" style={{ color: 'var(--color-text)' }}>{cat.name}</h3>
              <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>{count} produk</p>

              <div className="mt-4 h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--color-border)' }}>
                <div className="h-full rounded-full transition-all duration-500"
                  style={{ width: `${products.length > 0 ? (count / products.length) * 100 : 0}%`, background: cat.color }} />
              </div>
            </div>
          )
        })}
        ...
        {categories.length === 0 && (
          <div className="col-span-full text-center py-16" style={{ color: 'var(--color-text-muted)' }}>
            <svg className="w-14 h-14 mx-auto mb-4" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor">
...
function CategoryModal({ onClose, onSave }: { onClose: () => void, onSave: (name: string, icon: string, color: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9000] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative rounded-2xl w-full max-w-md shadow-2xl" style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)' }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5" style={{ borderBottom: '1px solid var(--color-border)' }}>
          <h2 className="text-lg font-bold" style={{ color: 'var(--color-text)' }}>Tambah Kategori</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="space-y-1.5">
            <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--color-text-muted)' }}>Nama Kategori</label>
            <input type="text" required value={name} onChange={e => setName(e.target.value)} className="form-input" placeholder="Nama kategori" />
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--color-text-muted)' }}>Icon</label>
            <select value={icon} onChange={e => setIcon(e.target.value)} className="form-input">
              {iconOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
            </select>
          </div>
          <div className="space-y-1.5">
            <label className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--color-text-muted)' }}>Warna</label>
            <div className="flex gap-2 items-center">
              <input type="color" value={color} onChange={e => setColor(e.target.value)} className="w-10 h-10 rounded-lg cursor-pointer" style={{ border: '1px solid var(--color-border)', background: 'transparent' }} />
