...
function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-[#EDEADE]">
        <Loader2 className="w-6 h-6 text-[#072C2C] animate-spin" />
      </div>
    )
  }
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header
          className="h-14 flex items-center justify-between px-4 md:px-6 shrink-0 border-b border-gray-200"
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            {user && (
              <button
                onClick={signOut}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium text-gray-500 hover:text-red-600 hover:bg-red-50 border border-transparent hover:border-red-200 transition-all"
                title="Logout"
              >
