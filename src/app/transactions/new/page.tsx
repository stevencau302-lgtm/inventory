        ...
        <div className="w-full max-w-5xl">
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 min-w-0">
              <div className="rounded-lg p-6 sm:p-8" style={{ background: '#1a1a1a', border: '3px solid #3f3f46', boxShadow: '6px 6px 0px #3f3f46' }}>
                {/* Header */}
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-12 h-12 rounded-md flex items-center justify-center" style={{ background: '#FDC800', border: '2px solid #3f3f46' }}>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  {/* Type */}
                  <div>
                    <label className="text-xs font-bold uppercase tracking-wider mb-3 block" style={{ color: '#a1a1aa', fontFamily: 'JetBrains Mono, monospace' }}>Tipe Transaksi</label>
                    <div className="grid grid-cols-2 gap-3">
                      <button type="button" onClick={() => setType('in')} className="p-4 rounded-md text-left transition-all" style={{ background: type === 'in' ? '#16A34A' : '#1a1a1a', border: '2px solid #3f3f46', boxShadow: type === 'in' ? '3px 3px 0px #3f3f46' : '2px 2px 0px #3f3f46', color: type === 'in' ? '#fff' : '#e4e4e7' }}>
                        <p className="text-sm font-bold">Masuk</p>
                        <p className="text-xs mt-0.5" style={{ opacity: 0.7 }}>Stok bertambah</p>
                      </button>
                      <button type="button" onClick={() => setType('out')} className="p-4 rounded-md text-left transition-all" style={{ background: type === 'out' ? '#DC2626' : '#1a1a1a', border: '2px solid #3f3f46', boxShadow: type === 'out' ? '3px 3px 0px #3f3f46' : '2px 2px 0px #3f3f46', color: type === 'out' ? '#fff' : '#e4e4e7' }}>
                  ...
                  {/* Product */}
                  <div>
                    <label className="text-xs font-bold uppercase tracking-wider mb-2 block" style={{ color: '#a1a1aa', fontFamily: 'JetBrains Mono, monospace' }}>Pilih Produk</label>
                    <div className="relative" ref={productDropdownRef}>
                      <input type="text" value={productSearch} onChange={e => { setProductSearch(e.target.value); setShowProductDropdown(true); setSelectedProduct('') }} onFocus={() => setShowProductDropdown(true)} className="w-full rounded-md text-sm px-4 py-3 font-medium focus:outline-none" style={{ background: '#0f0f0f', border: '2px solid #3f3f46', color: '#fafafa' }} placeholder="Cari nama produk atau SKU..." autoComplete="off" />
                      {showProductDropdown && (
                        <div className="absolute z-50 left-0 right-0 top-full mt-1 rounded-md overflow-hidden max-h-56 overflow-y-auto" style={{ background: '#1a1a1a', border: '2px solid #3f3f46', boxShadow: '4px 4px 0px #3f3f46' }}>
                          {filteredProducts.length === 0 ? (
                            ...
                            <button key={p.id} type="button" onClick={() => handleSelectProduct(p.id)} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[#FDC800]/10" style={{ borderBottom: '1px solid #27272a' }}>
                              <div className="w-8 h-8 rounded-md flex items-center justify-center text-[10px] font-black shrink-0" style={{ background: '#432DD7', color: '#fff', border: '1.5px solid #3f3f46' }}>{p.name.substring(0, 2).toUpperCase()}</div>
                              <div className="flex-1 min-w-0">
                  ...
                  {/* Selected Product */}
                  {selected && (
                    <div className="rounded-md p-4" style={{ background: '#432DD7', border: '2px solid #3f3f46', boxShadow: '3px 3px 0px #3f3f46', color: '#fff' }}>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wider mb-2 block" style={{ color: '#a1a1aa', fontFamily: 'JetBrains Mono, monospace' }}>Jumlah</label>
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-10 rounded-md flex items-center justify-center font-bold text-lg shrink-0" style={{ background: '#0f0f0f', border: '2px solid #3f3f46', color: '#fafafa' }}>-</button>
                        <input type="number" min={1} value={quantity} onChange={e => setQuantity(Math.max(1, +e.target.value))} className="flex-1 min-w-0 rounded-md text-center text-lg font-bold py-2 focus:outline-none" style={{ background: '#0f0f0f', border: '2px solid #3f3f46', color: '#fafafa' }} />
                        <button type="button" onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 rounded-md flex items-center justify-center font-bold text-lg shrink-0" style={{ background: '#0f0f0f', border: '2px solid #3f3f46', color: '#fafafa' }}>+</button>
                      </div>
                    </div>
                    <div>
                      <label className="text-xs font-bold uppercase tracking-wider mb-2 block" style={{ color: '#a1a1aa', fontFamily: 'JetBrains Mono, monospace' }}>Tanggal</label>
                      <input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full rounded-md text-sm px-4 py-3 font-medium focus:outline-none" style={{ background: '#0f0f0f', border: '2px solid #3f3f46', color: '#fafafa', colorScheme: 'dark' }} />
                    </div>
                  </div>

                  {/* Note */}
                  <div>
                    <label className="text-xs font-bold uppercase tracking-wider mb-2 block" style={{ color: '#a1a1aa', fontFamily: 'JetBrains Mono, monospace' }}>Catatan <span className="normal-case font-normal" style={{ color: '#71717a' }}>(opsional)</span></label>
                    <textarea value={note} onChange={e => setNote(e.target.value.slice(0, 200))} maxLength={200} rows={3} className="w-full rounded-md text-sm px-4 py-3 resize-none font-medium focus:outline-none" style={{ background: '#0f0f0f', border: '2px solid #3f3f46', color: '#fafafa' }} placeholder="Tambahkan catatan..." />
                    <p className="text-right text-[10px] mt-1" style={{ color: '#71717a', fontFamily: 'JetBrains Mono, monospace' }}>{note.length}/200</p>
                  </div>

                  {/* Submit */}
                  <div className="flex gap-3 pt-2">
                    <button type="button" onClick={() => router.push('/transactions')} className="px-5 py-3 rounded-md text-sm font-bold transition-all" style={{ background: '#0f0f0f', border: '2px solid #3f3f46', color: '#e4e4e7', boxShadow: '2px 2px 0px #3f3f46' }}>Batal</button>
                    <button type="submit" disabled={loading} className="flex-1 py-3.5 rounded-md text-sm font-bold transition-all disabled:opacity-50 flex items-center justify-center gap-2" style={{ background: '#FDC800', border: '2px solid #3f3f46', color: '#1C293C', boxShadow: '4px 4px 0px #3f3f46' }}>
            ...
            <div className="lg:w-[300px] shrink-0">
              <div className="lg:sticky lg:top-8 space-y-4">
                <div className="rounded-lg overflow-hidden" style={{ background: '#1a1a1a', border: '3px solid #3f3f46', boxShadow: '4px 4px 0px #3f3f46' }}>
                  <div className="px-5 py-3.5 flex items-center gap-2 font-bold text-sm" style={{ background: '#FDC800', borderBottom: '2px solid #3f3f46', color: '#1C293C' }}>
                  ...
                  {selected ? (
                    <div className="px-5 py-4 space-y-3">
                      <div className="flex items-center gap-3 pb-3" style={{ borderBottom: '1px solid #27272a' }}>
                        <div className="w-9 h-9 rounded-md flex items-center justify-center text-[10px] font-black" style={{ background: '#432DD7', color: '#fff', border: '1.5px solid #3f3f46' }}>{selected.name.substring(0, 2).toUpperCase()}</div>
                        <div className="flex-1 min-w-0">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between"><span className="text-xs" style={{ color: '#71717a' }}>Tipe</span><span className="text-xs font-bold px-2 py-0.5 rounded" style={{ background: type === 'in' ? '#16A34A' : '#DC2626', color: '#fff' }}>{type === 'in' ? 'Masuk' : 'Keluar'}</span></div>
                      ...
                      <div className="rounded-md p-3 mt-2" style={{ background: '#0f0f0f', border: '1.5px solid #3f3f46' }}>
                    ...
                    <div className="px-5 py-8 text-center">
                      <div className="w-12 h-12 rounded-md mx-auto mb-3 flex items-center justify-center" style={{ background: '#0f0f0f', border: '2px solid #27272a' }}>
