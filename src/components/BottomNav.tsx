...
const navItems = [
  ...
  { href: '/stock-opname', label: 'Opname', icon: 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
...
export default function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[9999] bg-white border-t border-gray-200 shadow-[0_-2px_10px_rgba(0,0,0,0.06)]">
      <div className="flex items-center justify-around h-[64px] px-2 max-w-[500px] mx-auto">
        {navItems.map(item => {
          const isActive = pathname === item.href || (item.href !== '/' && pathname.startsWith(item.href))
          return (
            <Link key={item.href} href={item.href}
              className="flex flex-col items-center justify-center gap-0.5 py-2 min-w-[56px] active:scale-95 transition-transform">
              {/* Active top indicator */}
              <div className={`w-6 h-[2.5px] rounded-full mb-0.5 transition-all ${isActive ? 'bg-[#072C2C]' : 'bg-transparent'}`} />
              <svg
