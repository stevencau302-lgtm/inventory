    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[9999] pb-[env(safe-area-inset-bottom,0px)]" style={{ position: 'fixed' }}>
      <div className="flex items-center justify-around h-16 px-2 max-w-[390px] mx-auto mx-4 mb-3 rounded-2xl border border-white/[0.06]"
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`flex flex-col items-center gap-1 py-1 px-3 transition relative ${isActive ? 'text-indigo-400' : 'text-zinc-500'}`}>
              ...
              {isActive && <span className="absolute -bottom-1 w-5 h-[3px] rounded-full bg-indigo-400 shadow-[0_0_8px_rgba(99,102,241,0.5)]" />}
            </Link>
          )
        })}
      </div>
