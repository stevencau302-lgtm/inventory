...
export default function BottomNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-[9999]" style={{ background: '#1a1a1a', borderTop: '1px solid #2a2a2a' }}>
        ...
        {navItems.map(item => {
          if (item.label === 'add') return (
            <Link key={item.href} href={item.href} className="relative -top-4">
              <div className="w-13 h-13 rounded-2xl flex items-center justify-center active:scale-95 transition"
                style={{ background: '#FDC800', boxShadow: '0 4px 16px rgba(253, 200, 0, 0.35)', width: '52px', height: '52px' }}>
          ...
          return (
            <Link key={item.href} href={item.href} className="flex flex-col items-center gap-1 py-2 px-3 transition">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={isActive ? 2 : 1.5} stroke="currentColor" style={{ color: isActive ? '#FDC800' : '#6B7280' }}><path strokeLinecap="round" strokeLinejoin="round" d={item.icon} /></svg>
              <span className="text-[10px] font-medium" style={{ color: isActive ? '#FDC800' : '#6B7280' }}>{item.label}</span>
            </Link>
          )
        })}
      </div>
