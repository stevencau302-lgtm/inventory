...
// Ghost chart bars for premium empty state background
function GhostChart() {
  return (
  ...
  const barData = hasOutData
    ? [...products]
        .map(p => ({
          ...
          penjualan: outTransactions.filter(t => t.productId === p.id).reduce((s, t) => s + t.quantity, 0),
        }))
    ...
    ? (() => {
        ...
        for (let i = 6; i >= 0; i--) {
          ...
          result.push({
            ...
            masuk: transactions.filter(t => t.type === 'in' && t.createdAt.startsWith(dayStr)).reduce((s, t) => s + t.quantity, 0),
            keluar: transactions.filter(t => t.type === 'out' && t.createdAt.startsWith(dayStr)).reduce((s, t) => s + t.quantity, 0),
          })
        }
        return result
      })()
