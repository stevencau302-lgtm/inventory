  ...
  return (
    <div className="relative flex flex-col items-center justify-center h-[280px] px-6">
      <div className="relative z-10 flex flex-col items-center text-center">
        ...
        <h4 className="text-[15px] font-semibold text-gray-800 mb-1.5">Belum ada data pendapatan</h4>
        <p className="text-[12px] text-gray-500 leading-relaxed max-w-[240px] mb-5">
          Grafik pendapatan akan muncul setelah transaksi keluar pertama dicatat.
        </p>
...
export default function DashboardCharts({ products, transactions }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      {/* Area Chart - Pendapatan Harian (2/3 width like reference) */}
      <div className="lg:col-span-2 rounded-xl bg-white border border-gray-200 overflow-hidden">
        <div className="px-5 pt-5 pb-2 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Pendapatan Harian</h3>
            <p className="text-[11px] text-gray-500 mt-0.5">per jam · ribuan rupiah</p>
          </div>
            ...
            <div className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={areaData} margin={{ top: 15, right: 20, left: 0, bottom: 10 }}>
                  ...
                  <YAxis stroke="#9ca3af" fontSize={11} tickFormatter={(v) => `${v}rb`} />
                  <Tooltip
                    contentStyle={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', color: '#111827', fontSize: '12px' }}
                    labelStyle={{ color: '#6b7280' }}
                    formatter={(value: number) => [`Rp ${(value * 1000).toLocaleString('id-ID')}`, 'Pendapatan']}
                  />
                  <Area type="monotone" dataKey="pendapatan" stroke="#FF5F03" strokeWidth={2.5} fill="url(#colorPendapatan)" dot={{ fill: '#FF5F03', r: 3, strokeWidth: 0 }} activeDot={{ r: 5, fill: '#FF5F03', stroke: '#fff', strokeWidth: 2 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          ) : (
